...
public class PlayerCharacter : Controller
{
    ...
    void FixedUpdate()
    {
        ...
        //Handle jumping;
        if ((playerInput != null) && isGrounded && playerInput.IsJumpKeyPressed())
        {
            OnJumpStart();
    ...
    private Vector3 CalculateMovementDirection()
    {
        ...
        {
            _direction += tr.right * playerInput.GetHorizontalMovementInput();
            _direction += tr.forward * playerInput.GetVerticalMovementInput();
        }
        else
        {
            ...
            _direction += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized *
                          playerInput.GetHorizontalMovementInput();
            _direction += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized *
                          playerInput.GetVerticalMovementInput();
        }
